...
### Fixed
- **watchdog 僵尸态误判**（2026-09-25 夜间实际发生一次，白重启了健康的 vector）：
  原判据"10 分钟零入库即僵尸"是绝对阈值，当初按 corosync 洪水期（~5,300 条/小时）
  设定；洪水停止后基线降到 ~150 条/小时，夜间单窗口零日志属正常波动。
  改为**连续 3 个零流量窗口（约 30 分钟）**才判僵尸；单窗口零流量记为 `quiet`
  状态（徽章仍蓝、悬停显示"零入库观察中 n/3"），不触发任何动作。
  实现上保证每个窗口最多计一次，避免页面/巡检频繁调用 probe() 重复计数
- `scripts/version.sh` 每次升版本在 CHANGELOG 顶部累积一个空行（已累积 13 个）；
  去掉 awk 的 `BEGIN{print ""}` 并清理现存空行


### Changed
- 采集器徽章二态化：**连接 = 蓝系底色**（进程在跑，含降级），**未连接 = 灰系底色**；
  状态细节（断流/上次失败/SMB 丢失）保留在悬停提示里
...
### Changed
- 总览图表卡片标题改为小徽章（比原 h3 略小）：「各服务器日志量」→ 点击跳纳管页、
  「各服务器错误数」→ 点击跳 error 日志搜索（亮色）；「日志量趋势」「等级分布」无链接（灰色）
- 去掉页头 logo 旁的绿色脉冲圆点
